...
doforward() {
    ...
    port=$2
    remotePort=$3
    pod=$(kubectl -n ftgo get pods --selector=svc=$service  -o jsonpath='{.items[*].metadata.name}')
    echo "$service $pod $port"
    kubectl -n ftgo port-forward svc/$service ${port}:${remotePort} &
...
doforward 'ftgo-api-gateway' 8087 80
